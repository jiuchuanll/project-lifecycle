...
var deriveAlignmentReview = ({ feedbacks = [], owners = [], closures = [] } = {}) => {
  ...
  const rows = [];
  for (const [index, record3] of feedbacks.entries()) {
    const validation2 = validateJson("delivery-frontmatter", record3?.frontmatter);
    if (!validation2.ok || record3.frontmatter.artifact_kind !== "feedback" || record3.frontmatter.retention_tier !== "active") {
      return failure3(`/feedbacks/${index}`, "Alignment entries must reference valid Feedback Frontmatter.");
    }
    const feedbackId = record3.frontmatter.artifact_id;
    if (ids.has(feedbackId)) return failure3(`/feedbacks/${index}`, "Feedback IDs must be unique.");
    ids.add(feedbackId);
    if (record3.marker === null) continue;
    if (!validateJson("alignment-marker", record3.marker).ok || record3.marker.primary_domain_id !== record3.frontmatter.domain_ids.find((id) => id === record3.marker.primary_domain_id) || !safeTitle(record3.titles?.en) || !safeTitle(record3.titles?.["zh-CN"])) {
    ...
    for (const candidate of linkedOwners) {
      const feedbackProject = record3.frontmatter.current_project_id ?? record3.frontmatter.project_id_at_creation;
    ...
    const alignmentPhase = record3.marker.routing_disposition === "DEFERRED" ? "DEFERRED" : ownerRef.length > 0 && !allAccepted ? "DELIVERY_OPEN" : allAccepted ? "KNOWLEDGE_WRITEBACK" : "REVIEW_REQUIRED";
    rows.push({
      feedback_id: feedbackId,
      title: { en: record3.titles.en, "zh-CN": record3.titles["zh-CN"] },
      primary_domain_id: record3.marker.primary_domain_id,
