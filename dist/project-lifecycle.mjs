...
import { open as open4, readFile as readFile6, realpath as realpath7, stat } from "node:fs/promises";
import { basename as basename3, dirname as dirname4, isAbsolute as isAbsolute8, relative as relative6, resolve as resolve5, sep as sep6 } from "node:path";
...
function isRawAbsolute(candidate) {
  return isAbsolute(candidate) || candidate.startsWith("\\") || /^[A-Za-z]:[\\/]/u.test(candidate);
}
...
async function atomicWriteValidated({ root, target, content, validate }) {
  ...
  const temporaryPath = join2(
    dirname2(targetPath),
    `.${basename2(targetPath)}.${process.pid}.tmp`
  );
  let temporaryHandle;
...
var createError = (code, path, message) => ({ code, path, message });

// scripts/lib/bilingual-pair.mjs
import { readFile as readFile2, realpath as realpath2 } from "node:fs/promises";
import { dirname as dirname3, isAbsolute as isAbsolute2, normalize, relative as relative2, resolve as resolve2, sep as sep2 } from "node:path";
import { fileURLToPath } from "node:url";

// scripts/lib/deterministic-order.mjs
...
var codePointOrderErrors = (items, {
...
// scripts/lib/result.mjs
var ok = (value = null) => ({ ok: true, value, errors: [] });
var fail = (errors) => ({ ok: false, value: null, errors });
...
var alignment_resolution_schema_default = {
  ...
  properties: {
    ...
    knowledge_resolution_refs: {
      ...
      maxItems: 20,
      uniqueItems: true,
      items: { type: "string", maxLength: 500, pattern: "^knowledge-resolution:[a-z][a-z0-9-]*$" }
    },
    human_approval_ref: { type: "string", minLength: 1, maxLength: 500, pattern: "^[^\\s,;；，]+$" }
  },
  allOf: [
    {
...
var archive_access_receipt_schema_default = {
  ...
  properties: {
    ...
    reason: { enum: ["AUDIT", "EXPLICIT_ID", "HISTORICAL_COMPARISON", "INCIDENT", "REGRESSION"] },
    question: { type: "string", minLength: 1, maxLength: 1e3 },
    insufficiency_reason: { type: "string", minLength: 1, maxLength: 1e3 },
    artifact_ids: {
      type: "array",
      minItems: 1,
      maxItems: 20,
...
// scripts/schemas/capability-frontmatter.schema.json
var capability_frontmatter_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "capability-frontmatter",
  type: "object",
  additionalProperties: false,
  required: [
    "id",
  ...
  properties: {
    id: { type: "string", pattern: "^[a-z][a-z0-9-]*$" },
    knowledge_state: { enum: ["current", "in-progress", "proposed", "superseded"] },
    paired_asset: { type: "string", minLength: 1 },
    last_verified_baseline: { type: "string", minLength: 1 },
    implementation_refs: {
      type: "array",
      uniqueItems: true,
  ...
  if: {
    properties: { knowledge_state: { const: "current" } },
    required: ["knowledge_state"]
  },
  then: {
    properties: {
      implementation_refs: { type: "array", minItems: 1 },
      verification_refs: { type: "array", minItems: 1 }
    }
  }
};

// scripts/schemas/context-receipt.schema.json
var context_receipt_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "context-receipt",
  type: "object",
  additionalProperties: false,
  ...
  properties: {
    ...
    knowledge_baseline: { $ref: "#/$defs/ref" },
    intent_summary: { type: "string", minLength: 1, maxLength: 500 },
    route: {
      type: "object",
      additionalProperties: false,
      required: ["primary_domain_id", "affected_domain_ids"],
      properties: {
        primary_domain_id: { $ref: "#/$defs/id" },
        affected_domain_ids: {
          type: "array",
          maxItems: 50,
          uniqueItems: true,
    ...
    selected_context: {
      type: "array",
      maxItems: 100,
      items: { $ref: "#/$defs/selectedContext" }
    },
    material_exclusions: {
      type: "array",
      maxItems: 100,
      uniqueItems: true,
      items: { $ref: "#/$defs/materialExclusion" }
    },
    open_questions: {
      type: "array",
      maxItems: 50,
    ...
    stop: {
      ...
      properties: {
        code: {
          enum: ["SUFFICIENT", "NEEDS_USER", "NEEDS_EVIDENCE", "NEEDS_ARCHIVE_RECEIPT", "CONFLICT"]
        },
      ...
      then: {
        ...
        properties: {
          stop: {
            type: "object",
            properties: {
              code: { enum: ["NEEDS_USER", "NEEDS_EVIDENCE", "NEEDS_ARCHIVE_RECEIPT", "CONFLICT"] }
            }
          }
        }
      }
    },
    {
      if: {
        ...
        properties: {
          stop: {
            type: "object",
            properties: { code: { const: "NEEDS_USER" } },
            required: ["code"]
          }
        },
        required: ["stop"]
      ...
      then: {
        type: "object",
        properties: { open_questions: { type: "array", minItems: 1 } }
      }
    }
  ],
  $defs: {
    ...
    selectedContext: {
      type: "object",
      additionalProperties: false,
      required: ["kind", "id", "version_ref", "reason"],
      properties: {
        kind: { enum: ["global_principle", "domain_asset", "fact", "constraint", "active_delivery"] },
        id: { $ref: "#/$defs/id" },
        version_ref: { $ref: "#/$defs/ref" },
        reason: { enum: ["PRIMARY", "DEPENDENCY", "CONSTRAINT", "ACTIVE_CHANGE", "USER_EXPLICIT", "VALIDATION"] }
      }
    },
    materialExclusion: {
      type: "object",
      additionalProperties: false,
      required: ["id", "reason", "explanation"],
      properties: {
        id: { $ref: "#/$defs/id" },
        reason: { enum: ["OUT_OF_SCOPE", "REDUNDANT", "UNCONFIRMED", "STALE", "ARCHIVE_GATED"] },
...
var delivery_frontmatter_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "delivery-frontmatter",
  type: "object",
  additionalProperties: false,
  required: [
    "schema_version",
  ...
  properties: {
    ...
    primary_route: { enum: ["KNOWLEDGE_UPDATE", "NON_PRD_DELIVERY", "OUTSIDE_PLUGIN", "PRD_DELIVERY"] },
    project_id_at_creation: { $ref: "#/$defs/id" },
    current_project_id: { $ref: "#/$defs/id" },
    domain_ids: {
      type: "array",
      minItems: 1,
      maxItems: 50,
    ...
    relationships: {
      type: "object",
      additionalProperties: false,
      required: ["feedback_ids", "prd_ids", "legacy_artifact_refs"],
      properties: {
        feedback_ids: { $ref: "#/$defs/feedbackIds" },
        prd_ids: { $ref: "#/$defs/prdIds" },
        legacy_artifact_refs: { $ref: "#/$defs/refs" }
      }
    },
    retention_tier: { enum: ["active", "archive", "closed-summary"] },
    reclassified_from_refs: {
      type: "array",
      maxItems: 20,
      uniqueItems: true,
  ...
  allOf: [
    ...
    },
    {
      not: {
        type: "object",
        properties: { artifact_id: { enum: ["alignment-review", "alignment-review-en"] } },
    ...
    },
    {
      ...
      then: {
        type: "object",
        required: ["current_project_id"],
        properties: { current_project_id: { $ref: "#/$defs/id" } }
      },
      else: { type: "object", not: { type: "object", required: ["current_project_id"] } }
    },
    {
      if: { type: "object", properties: { artifact_kind: { const: "feedback" } }, required: ["artifact_kind"] },
      then: { type: "object", properties: { artifact_id: { type: "string", pattern: "^feedback-[a-z0-9-]+$" } } }
    },
    {
      if: { type: "object", properties: { artifact_kind: { const: "prd" } }, required: ["artifact_kind"] },
      then: { type: "object", properties: { artifact_id: { type: "string", pattern: "^prd-[a-z0-9-]+$" } } }
    }
  ],
  $defs: {
    id: { type: "string", pattern: "^[a-z][a-z0-9-]*$" },
    ...
    feedbackIds: {
      ...
      maxItems: 100,
      uniqueItems: true,
      items: { type: "string", pattern: "^feedback-[a-z0-9-]+$" }
    },
    prdIds: {
      type: "array",
      maxItems: 100,
      uniqueItems: true,
      items: { type: "string", pattern: "^prd-[a-z0-9-]+$" }
    },
    refs: {
      type: "array",
      maxItems: 100,
...
var knowledge_diff_schema_default = {
  ...
  properties: {
    ...
    domain_changes: {
      ...
      maxItems: 50,
      uniqueItems: true,
      items: { $ref: "#/$defs/domainChange" }
    },
    entry_points: { $ref: "#/$defs/refs" },
    ...
    outcome: { enum: ["CHANGE", "NO_CHANGE"] }
  },
  allOf: [
    {
      if: { type: "object", properties: { outcome: { const: "NO_CHANGE" } }, required: ["outcome"] },
      then: {
        type: "object",
        properties: {
          operations: { type: "array", maxItems: 0 },
          domain_changes: { type: "array", maxItems: 0 },
          evidence_refs: { type: "array", minItems: 1 }
        }
      }
    },
    {
      if: { type: "object", properties: { outcome: { const: "CHANGE" } }, required: ["outcome"] },
      then: {
        type: "object",
        anyOf: [
          { type: "object", properties: { operations: { type: "array", minItems: 1 } } },
          { type: "object", properties: { domain_changes: { type: "array", minItems: 1 } } }
        ]
      }
    }
  ],
  $defs: {
    ...
    operation: {
      type: "object",
      additionalProperties: false,
      required: ["kind", "fact_id", "owner_domain_id", "evidence_refs"],
      properties: {
        kind: { enum: ["ADD", "REWRITE", "SUPERSEDE"] },
        fact_id: { $ref: "#/$defs/id" },
        owner_domain_id: { $ref: "#/$defs/id" },
        successor_fact_id: { $ref: "#/$defs/id" },
        evidence_refs: { $ref: "#/$defs/refs" }
      },
      allOf: [
        {
          if: { type: "object", properties: { kind: { const: "SUPERSEDE" } }, required: ["kind"] },
          then: {
            type: "object",
            required: ["successor_fact_id"],
            properties: { successor_fact_id: { $ref: "#/$defs/id" } }
          }
        }
      ]
    },
    domainChange: {
      type: "object",
      additionalProperties: false,
      required: ["domain_id", "change", "evidence_refs"],
      properties: {
        domain_id: { $ref: "#/$defs/id" },
        change: { type: "string", minLength: 1, maxLength: 1e3 },
        relationship_refs: { $ref: "#/$defs/refs" },
        evidence_refs: { $ref: "#/$defs/refs" }
      }
    }
  }
...
var obligation_instance_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "obligation-instance",
  type: "object",
  additionalProperties: false,
  required: [
    "obligation_id",
  ...
  properties: {
    ...
    status: { enum: ["OPEN", "RESOLVED", "SUPERSEDED", "WAIVED"] },
    trigger_refs: { $ref: "#/$defs/nonEmptyRefs" },
    scope_refs: { $ref: "#/$defs/nonEmptyRefs" },
    responsible_refs: { $ref: "#/$defs/nonEmptyRefs" },
    required_before: { $ref: "#/$defs/ref" },
    evidence_refs: { $ref: "#/$defs/refs" },
    resolution_ref: { $ref: "#/$defs/ref" },
    human_approval_ref: { $ref: "#/$defs/ref" },
  ...
  allOf: [
    {
      if: { type: "object", properties: { status: { const: "OPEN" } }, required: ["status"] },
      then: {
        type: "object",
        properties: {
          resolution_ref: false,
          human_approval_ref: false,
          successor_obligation_ref: false
        }
      }
    },
    {
      if: { type: "object", properties: { status: { const: "RESOLVED" } }, required: ["status"] },
      then: {
        type: "object",
    ...
    {
      if: { type: "object", properties: { status: { const: "WAIVED" } }, required: ["status"] },
      then: {
        type: "object",
        required: ["human_approval_ref"],
        properties: {
    ...
    {
      if: { type: "object", properties: { status: { const: "SUPERSEDED" } }, required: ["status"] },
      then: {
        type: "object",
        required: ["successor_obligation_ref"],
        properties: {
          resolution_ref: false,
          human_approval_ref: false,
          successor_obligation_ref: {
...
// scripts/schemas/pending-changes.schema.json
var pending_changes_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "pending-changes",
  type: "object",
  additionalProperties: false,
  ...
  $defs: {
    ...
    change: {
      oneOf: [
        { $ref: "#/$defs/legacyChange" },
        { $ref: "#/$defs/governedChange" },
        { $ref: "#/$defs/absorptionChange" }
      ]
    },
    absorptionChange: {
      type: "object",
      ...
      properties: {
        absorption_version: { const: 1 },
        change_id: { type: "string", pattern: "^[a-z][a-z0-9-]*$" },
        semantic_target_key: { type: "string", pattern: "^(fact|domain|owner|constraint|relationship|topology):[a-z][a-z0-9-]*$" },
        conflict_revision: { type: "integer", minimum: 1 },
        candidate_commitment: { type: "string", pattern: "^sha256:[0-9a-f]{64}$" },
        diff_id: { $ref: "#/$defs/id" },
        owner_delivery_id: { $ref: "#/$defs/id" },
        knowledge_baseline: { type: "string", minLength: 1, maxLength: 500 },
        new_baseline: { type: "string", minLength: 1, maxLength: 500 },
        operations: {
          type: "array",
          maxItems: 100,
          items: { $ref: "#/$defs/absorptionOperation" }
        },
        affected_domain_ids: { $ref: "#/$defs/ids" },
        affected_fact_ids: { $ref: "#/$defs/ids" },
        affected_owner_ids: { $ref: "#/$defs/ids" },
        constraint_refs: { $ref: "#/$defs/list" },
        relationship_refs: { $ref: "#/$defs/list" },
        topology_target_ids: { $ref: "#/$defs/ids" },
        evidence_refs: { $ref: "#/$defs/list" },
        risks: { $ref: "#/$defs/boundedNotes" },
        evidence_gaps: { $ref: "#/$defs/boundedNotes" },
        review_state: { const: "open" },
      ...
      allOf: [
        {
          ...
          else: { properties: { successor_fact_id: false } }
        }
      ]
    },
    boundedNotes: {
    ...
    legacyChange: {
      ...
      properties: {
        ...
        trigger_refs: { $ref: "#/$defs/refs" },
        affected_refs: { $ref: "#/$defs/refs" },
        proposed_disposition: { type: "string", minLength: 1 },
        risks: { $ref: "#/$defs/notes" },
        evidence_gaps: { $ref: "#/$defs/notes" },
        review_state: { const: "open" },
    ...
    governedChange: {
      ...
      properties: {
        ...
        semantic_target_key: { type: "string", pattern: "^(constraint|domain|relationship|exception):[a-z][a-z0-9-]*(?::[a-z][a-z0-9-]*)?$" },
        source_refs: { $ref: "#/$defs/refs" },
        baseline: { $ref: "#/$defs/baseline" },
        change_class: { enum: ["REPLACEMENT", "SEMANTIC", "WORDING"] },
        proposed_patch: { $ref: "#/$defs/proposedPatch" },
        child_dispositions: {
          type: "array",
          items: { $ref: "#/$defs/childDisposition" }
        },
        knowledge_commitments: {
          type: "array",
          items: { $ref: "#/$defs/knowledgeCommitment" }
        },
        obligations: { $ref: "#/$defs/obligations" }
      }
    },
    ...
    baseline: {
      ...
      required: ["map_hash"],
      properties: {
        map_hash: { type: "string", pattern: "^sha256:[0-9a-f]{64}$" }
      }
    },
    proposedPatch: {
      type: "object",
      ...
      properties: {
        ...
        target_type: { enum: ["constraint", "domain", "exception", "relationship"] },
        ...
        candidate_map_hash: { type: "string", pattern: "^sha256:[0-9a-f]{64}$" },
        expected_semantic_revision: { type: "integer", minimum: 1 },
        new_ids: { type: "array", uniqueItems: true, items: { $ref: "#/$defs/id" } },
        successor_ids: { type: "array", uniqueItems: true, items: { $ref: "#/$defs/id" } }
      }
    },
    childDisposition: {
      type: "object",
      additionalProperties: false,
      required: ["domain_id", "disposition", "evidence_refs", "unresolved_fact_ids"],
      properties: {
        domain_id: { $ref: "#/$defs/id" },
        disposition: { enum: ["EXCEPTION", "MERGE", "NO_CHANGE", "REPARENT", "REVALIDATE", "RETIRE", "SPLIT"] },
        target_id: { $ref: "#/$defs/id" },
        exception_ref: { type: "string", minLength: 1 },
        evidence_refs: { $ref: "#/$defs/refs" },
        unresolved_fact_ids: { type: "array", uniqueItems: true, items: { $ref: "#/$defs/id" } }
      }
    },
    knowledgeCommitment: {
      type: "object",
      additionalProperties: false,
      required: ["domain_id", "en", "zh-CN", "facts"],
      properties: {
        domain_id: { $ref: "#/$defs/id" },
        en: { $ref: "#/$defs/contentCommitment" },
        "zh-CN": { $ref: "#/$defs/contentCommitment" },
        facts: {
          type: "array",
          items: { $ref: "#/$defs/factSummary" }
        }
      }
    },
    contentCommitment: {
    ...
    factSummary: {
      type: "object",
      additionalProperties: false,
      required: ["fact_id", "fact_revision", "knowledge_state"],
      properties: {
        fact_id: { $ref: "#/$defs/id" },
        fact_revision: { type: "integer", minimum: 1 },
        knowledge_state: { enum: ["current", "proposed"] }
      }
    },
    id: {
      type: "string",
...
// scripts/schemas/project-extensions.schema.json
var project_extensions_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "project-extensions",
  type: "object",
  additionalProperties: false,
  required: ["schema_version", "project_id", "registry_revision", "secondary_obligation_kinds"],
  properties: {
    schema_version: { const: 1 },
...
// scripts/schemas/project-map.schema.json
var project_map_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "project-map",
  type: "object",
  additionalProperties: false,
  ...
  $defs: {
    ...
    projectIdentity: {
      type: "object",
      additionalProperties: false,
      required: ["label", "purpose", "calibration_ref"],
      properties: {
    ...
    identityLineage: {
      type: "object",
      additionalProperties: false,
      required: ["predecessor_project_id", "relationship", "successor_project_ids", "effective_baseline", "approval_ref"],
      properties: {
        predecessor_project_id: { $ref: "#/$defs/id" },
        relationship: { enum: ["SUCCESSOR", "SPLIT", "MERGE"] },
        successor_project_ids: { type: "array", minItems: 1, uniqueItems: true, items: { $ref: "#/$defs/id" } },
        effective_baseline: { type: "string", minLength: 1 },
        approval_ref: { type: "string", minLength: 1 },
        history_locator: { type: "string", minLength: 1 }
      }
    },
    repository: {
      type: "object",
      additionalProperties: false,
      required: ["id", "purpose", "portable_locator", "integration_ref", "domain_ids", "knowledge_asset_locators", "accepted_revision"],
      properties: {
        id: { $ref: "#/$defs/id" },
        purpose: { $ref: "#/$defs/localizedText" },
    ...
    scope: {
      ...
      properties: {
        includes: { type: "array", minItems: 1, uniqueItems: true, items: { type: "string", minLength: 1 } },
        excludes: { type: "array", uniqueItems: true, items: { type: "string", minLength: 1 } }
      }
    },
    relationship: {
      type: "object",
      ...
      properties: {
        kind: { enum: ["coordinates_with", "depends_on", "governed_by"] },
        target_id: { $ref: "#/$defs/id" }
      }
    },
    pairedAssets: {
    ...
    domain: {
      type: "object",
      additionalProperties: false,
      required: ["id", "kind", "label", "purpose", "domain_state", "scope", "parent_id", "relationships", "evidence_refs", "known_gaps"],
      properties: {
        id: { $ref: "#/$defs/id" },
        kind: { enum: ["capability", "domain", "project_module"] },
        label: { $ref: "#/$defs/localizedText" },
        purpose: { $ref: "#/$defs/localizedText" },
        domain_state: { enum: ["confirmed", "materialized", "merged", "retired"] },
        ...
        known_gaps: { type: "array", uniqueItems: true, items: { type: "string", minLength: 1 } },
        paired_assets: { $ref: "#/$defs/pairedAssets" },
    ...
    constraint: {
      ...
      properties: {
        id: { $ref: "#/$defs/id" },
        scope: { enum: ["descendants", "selected_descendants", "self"] },
        owner_id: { $ref: "#/$defs/id" },
        selected_descendants: { type: "array", minItems: 1, uniqueItems: true, items: { $ref: "#/$defs/id" } },
        semantic_revision: { type: "integer", minimum: 1 },
        lifecycle_state: { enum: ["current", "retired"] },
        knowledge_refs: { $ref: "#/$defs/knowledgeRefs" },
        successor_ids: {
          type: "array",
          minItems: 1,
          uniqueItems: true,
        ...
        retirement_reason_ref: { type: "string", minLength: 1 },
        exceptions: {
          type: "array",
          items: { $ref: "#/$defs/constraintException" }
        }
      },
      dependentRequired: {
        semantic_revision: ["owner_id", "lifecycle_state", "knowledge_refs"],
        lifecycle_state: ["owner_id", "semantic_revision", "knowledge_refs"],
        knowledge_refs: ["owner_id", "semantic_revision", "lifecycle_state"]
      },
      allOf: [{
        if: { properties: { scope: { const: "selected_descendants" } }, required: ["scope"] },
        then: {
          properties: {
            owner_id: { $ref: "#/$defs/id" },
            selected_descendants: { type: "array", minItems: 1, uniqueItems: true, items: { $ref: "#/$defs/id" } }
          },
          required: ["owner_id", "selected_descendants"]
        }
      }]
    },
    constraintException: {
      type: "object",
      additionalProperties: false,
      required: ["domain_id", "reason_ref", "approval_ref"],
      properties: {
        domain_id: { $ref: "#/$defs/id" },
    ...
    revalidationMarker: {
      type: "object",
      additionalProperties: false,
      required: ["domain_id", "fact_id", "reason_ref"],
      properties: {
        domain_id: { $ref: "#/$defs/id" },
        fact_id: { $ref: "#/$defs/id" },
        reason_ref: { type: "string", minLength: 1 },
      ...
      dependentRequired: {
        constraint_id: ["from_revision", "to_revision"],
        from_revision: ["constraint_id", "to_revision"],
        to_revision: ["constraint_id", "from_revision"]
      }
    }
  }
};

// scripts/schemas/project-pointer.schema.json
var project_pointer_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "project-pointer",
  type: "object",
  additionalProperties: false,
  required: ["schema_version", "project_id", "repository_id", "governance_locator"],
  properties: {
    schema_version: { const: 1 },
    project_id: { type: "string", pattern: "^[a-z][a-z0-9-]*$" },
    repository_id: { type: "string", pattern: "^[a-z][a-z0-9-]*$" },
    governance_locator: { type: "string", minLength: 1 },
    identity_migration_ref: { type: "string", minLength: 1 }
  }
};
...
var schemaValidators = new Map(
  [
    ...
    capability_frontmatter_schema_default,
    pending_changes_schema_default,
    context_receipt_schema_default,
...
// scripts/lib/markdown.mjs
var malformed = (path, message) => fail([
  createError("FACT_BLOCK_MALFORMED", path, message)
]);
var maskedLine = (line) => line.replace(/[^\n]/g, " ");
var maskFencedMarkdown = (source) => {
  let fence = null;
  return source.split(/(?<=\n)/).map((line) => {
    if (fence) {
      const closing = /^ {0,3}(`+|~+)[ \t]*(?:\n)?$/.exec(line);
...
// scripts/lib/fact-blocks.mjs
var OPEN = "<!-- project-lifecycle:fact";
...
var validateMachineFields = (value, index) => {
  ...
  for (const key of Object.keys(value)) {
    if (!MACHINE_FIELDS.includes(key)) errors.push(malformed2(`${path}/${key}`, `Unsupported fact field: ${key}`));
  }
  for (const field of MACHINE_FIELDS) {
    if (!(field in value)) errors.push(malformed2(`${path}/${field}`, `Missing fact field: ${field}`));
  }
  if (typeof value.fact_id !== "string" || !/^[a-z][a-z0-9-]*$/.test(value.fact_id)) {
    errors.push(malformed2(`${path}/fact_id`, "fact_id must be lowercase kebab-case."));
  }
  if (!Number.isInteger(value.revision) || value.revision < 1) {
    errors.push(malformed2(`${path}/revision`, "revision must be a positive integer."));
  }
  if (!Array.isArray(value.evidence_refs) || value.evidence_refs.some((item) => typeof item !== "string" || item.length === 0)) {
    errors.push(malformed2(`${path}/evidence_refs`, "evidence_refs must be an array of non-empty references."));
  } else if (new Set(value.evidence_refs).size !== value.evidence_refs.length) {
    errors.push(malformed2(`${path}/evidence_refs`, "evidence_refs must be unique."));
  } else {
  ...
  if (typeof value.last_verified_baseline !== "string" || value.last_verified_baseline.length === 0) {
    errors.push(malformed2(`${path}/last_verified_baseline`, "last_verified_baseline must be non-empty."));
  }
  return errors;
};
var parseHumanContent = (source, index) => {
  ...
  if (!limitsMatch) return { errors: [malformed2(`${path}/known_limits`, "A localized known-limits section is required.")] };
  ...
  const errors = [];
  if (!statement) errors.push(malformed2(`${path}/statement`, "A localized fact statement is required."));
  if (!knownLimits) errors.push(malformed2(`${path}/known_limits`, "Known limits must be explicit."));
  return { statement, knownLimits, errors };
};
var parseFactBlocks = (source) => {
  const normalized = source.replaceAll("\r\n", "\n");
  const facts = [];
  const errors = [];
  ...
  while (cursor < normalized.length) {
    ...
    if (strayCloseIndex !== -1 && (openIndex === -1 || strayCloseIndex < openIndex)) {
      errors.push(malformed2(`/facts/${facts.length}`, "Unmatched fact closing delimiter."));
      break;
    }
...
`, openIndex);
    if (!opensOnOwnLine) {
      errors.push(malformed2(path, "Fact opening delimiter must use the exact canonical form."));
      break;
    }
    const metadataEnd = normalized.indexOf("\n-->", openIndex + OPEN.length + 1);
    if (metadataEnd === -1) {
      errors.push(malformed2(path, "Fact metadata delimiter is unmatched."));
      break;
    }
    const metadataTerminatorEnd = metadataEnd + 4;
    if (normalized[metadataTerminatorEnd] !== "\n") {
      errors.push(malformed2(path, "Fact metadata terminator must use the exact canonical form."));
      break;
    }
    const closeIndex = normalized.indexOf(CLOSE, metadataTerminatorEnd + 1);
    if (closeIndex === -1) {
      errors.push(malformed2(path, "Fact closing delimiter is missing."));
      break;
    }
    ...
    if (!closesOnOwnLine) {
      errors.push(malformed2(path, "Fact closing delimiter must use the exact canonical form."));
      break;
    }
    const nestedIndex = normalized.indexOf(OPEN, metadataTerminatorEnd + 1);
    if (nestedIndex !== -1 && nestedIndex < closeIndex) {
      errors.push(malformed2(path, "Fact blocks cannot nest."));
      break;
    }
...
var pointerToken = (token) => String(token).replaceAll("~", "~0").replaceAll("/", "~1");
var propertyPath = (instancePath, property) => `${instancePath}/${pointerToken(property)}`;
var schemaErrors = (kind, errors) => errors.map((error) => {
  const path = error.keyword === "required" || error.keyword === "dependentRequired" ? propertyPath(error.instancePath, error.params.missingProperty) : error.keyword === "additionalProperties" ? propertyPath(error.instancePath, error.params.additionalProperty) : error.instancePath || "/";
...
var validateProjectMap = (value) => {
  const errors = [];
  const entries = namedEntries(value);
  const entriesById = /* @__PURE__ */ new Map();
  const domainById = /* @__PURE__ */ new Map();
  ...
  for (const [index, lineage] of value.identity_lineage.entries()) {
    ...
    if (lineage.predecessor_project_id === value.project_id || lineage.relationship === "SUCCESSOR" && lineage.successor_project_ids.length !== 1 || lineage.relationship === "SPLIT" && lineage.successor_project_ids.length < 2 || lineage.relationship === "MERGE" && lineage.successor_project_ids.length !== 1) {
  ...
  for (const [index, constraint] of value.constraints.entries()) {
    ...
    const exceptionDomains = /* @__PURE__ */ new Set();
  ...
  for (const [index, marker] of (value.revalidation_required ?? []).entries()) {
    ...
    if (marker.constraint_id) {
      ...
      } else if (marker.to_revision !== constraint.semantic_revision || marker.to_revision <= marker.from_revision) {
...
var validatePendingChanges = (value) => {
  const errors = [];
  ...
  for (const [index, change] of value.changes.entries()) {
    ...
    const commitmentIds = /* @__PURE__ */ new Set();
...
var validateProjectPointer = (value, options) => {
...
var validateJson = (kind, value, options = {}) => {
...
var FACT_FIELDS = ["fact_id", "revision", "evidence_refs", "last_verified_baseline"];
var toPath = (value) => value instanceof URL ? fileURLToPath(value) : resolve2(value);
var same = (left, right) => JSON.stringify(left) === JSON.stringify(right);
var relativeAssetLocator = (root, asset, pathApi = { relative: relative2, sep: sep2 }) => pathApi.relative(root, asset).split(pathApi.sep).join("/");
var inside = (root, candidate) => {
  const path = relative2(root, candidate);
  return path === "" || !path.startsWith("..") && !isAbsolute2(path);
};
var headingLevels = (body) => [...maskFencedMarkdown(body).matchAll(/^(#{1,6})[ \t]+.+$/gm)].map((match) => match[1].length);
var commonAssetRoot = (left, right) => {
  let root = dirname3(left);
  while (!inside(root, right)) root = dirname3(root);
  return root;
};
var locatorSegments = (locator) => normalize(locator).split(sep2).filter(Boolean);
var safeLocator = (locator) => typeof locator === "string" && locator.length > 0 && !isAbsolute2(locator) && !/^[A-Za-z]:[\\/]/.test(locator) && !locator.includes("\\") && !locator.includes("://") && !locator.split("/").includes("..") && locatorSegments(locator).length > 0;
var rootFromLocator = (assetPath, locator) => {
  let root = assetPath;
  for (const _segment of locatorSegments(locator)) root = dirname3(root);
  return resolve2(root, locator) === assetPath ? root : null;
};
var establishTrustedPair = async (enPath, zhPath, map) => {
  ...
  for (const domain of map.domains) {
    ...
    const enAsset = resolve2(enRoot, domain.paired_assets.en);
    const zhAsset = resolve2(enRoot, domain.paired_assets["zh-CN"]);
...
var readDocument = async (path) => {
  try {
    return { source: await readFile2(path, "utf8") };
  } catch {
    return { error: createError("PAIR_MACHINE_MISMATCH", "/frontmatter/paired_asset", "Paired capability asset is missing.") };
  }
};
var validateOwnedConstraintSections = ({ domain, map, sources }) => {
  const errors = [];
  const constraints = map.constraints.filter(({ knowledge_refs: refs, owner_id: ownerId }) => ownerId === domain.id && Boolean(refs));
    ...
    for (const language of ["en", "zh-CN"]) {
      ...
      if (valid) {
        ...
        for (let index = markerIndex + 1; valid && index < lines.length; index += 1) {
          ...
          if (line.startsWith('<a id="constraint-') || line.startsWith("<!-- project-lifecycle:constraint id=")) valid = false;
        }
        valid = valid && closed;
      }
      if (!valid) {
...
var validateBilingualPair = async (enPathValue, zhPathValue, map) => {
  ...
  const parseErrors = [enFrontmatter, zhFrontmatter, enFacts, zhFacts].filter((result) => !result.ok).flatMap((result) => result.errors);
  if (parseErrors.length > 0) return fail(parseErrors);
  const errors = [];
  for (const field of MACHINE_FIELDS2) {
  ...
  for (const [path, frontmatter] of [[enPath, enFrontmatter], [zhPath, zhFrontmatter]]) {
    const pairedPath = resolve2(dirname3(path), frontmatter.value.data.paired_asset);
    ...
    try {
      const realPairedPath = await realpath2(pairedPath);
  ...
  return errors.length > 0 ? fail(errors) : ok({ fact_ids: enFacts.value.map((fact) => fact.fact_id) });
};

// scripts/knowledge/collect-evidence.mjs
import { createHash } from "node:crypto";
import { lstat as lstat2, open as open2, opendir, realpath as realpath3 } from "node:fs/promises";
...
var instructionFiles = Object.freeze(["AGENTS.md", "CLAUDE.md", "CONTRIBUTING.md"]);
var overviewFiles = Object.freeze(["README", "README.md", "README.txt"]);
var manifestFiles = Object.freeze([
  "CMakeLists.txt",
  "Cargo.toml",
  "Dockerfile",
  "Gemfile",
...
var changelogFiles = Object.freeze(["CHANGELOG.md", "CHANGES.md", "HISTORY.md"]);
var topologyRoots = Object.freeze(["apps", "core", "modules", "packages", "ports", "src", "test", "tests"]);
var topologyContainers = /* @__PURE__ */ new Set(["apps", "core", "modules", "packages", "ports"]);
...
var evidenceError = (code, message) => {
  const error = new Error(message);
  error.code = code;
  return error;
};
var normalizeLimits = (provided = {}) => {
...
var portableLocator = (relativePath, fragment = "") => `repo:${relativePath}${fragment}`;
var contentHash = (observed) => `sha256:${createHash("sha256").update(JSON.stringify(observed)).digest("hex")}`;
var makeEntry = (kind, locator, observed) => ({
  kind,
  locator,
  observed,
  content_hash: contentHash(observed)
});
var pathSegmentsAreApproved = (relativePath) => relativePath.split("/").every((segment) => segment && !segment.startsWith(".") && !ignoredSegments.has(segment));
var safeStat = async (root, relativePath) => {
...
var addContentFile = async (entries, root, relativePath, kind, maxFileBytes) => {
...
var addPackageEntryPoints = (entries, manifestPath, observed) => {
  ...
  for (const name of names) {
    const command2 = manifest.scripts[name];
    if (!/^(build|deploy|dev|serve|start|test)(?::|$)/u.test(name) || typeof command2 !== "string") continue;
    entries.push(makeEntry(
      "declared_entry_point",
      portableLocator(manifestPath, `#scripts/${name}`),
      { name, command: command2 }
    ));
  }
};
var isRecognizedTopologyFile = (relativePath, name) => {
...
var collectTopology = async (root, maxTopologyEntries) => {
  const candidates = [];
...
async function collectEvidence({ root, limits: providedLimits = {} }) {
...
import { lstat as lstat3, readdir, readFile as readFile3, realpath as realpath4 } from "node:fs/promises";
import { isAbsolute as isAbsolute4, posix, relative as relative3, resolve as resolve3, sep as sep3 } from "node:path";
var JSON_PREFIX = "json:";
var PAIR_VALIDATOR = "bilingual-pair";
var PAIR_INPUTS = ["en", "zh-CN", "project_map"];
var POINTER_INPUTS = ["resolved_project_map"];
var ENTRY_FIELDS = ["expected_code", "inputs", "path", "validator"];
var MANIFEST_FIELDS = ["auxiliary_roots", "fixtures", "schema_version"];
var EXPECTED_CODES = /* @__PURE__ */ new Set([
  "OK",
...
var manifestError = (path) => ({ code: "FIXTURE_MANIFEST_INVALID", path });
var suiteError = () => ({
  ok: false,
  results: [],
  errors: [{ code: "FIXTURE_SUITE_ERROR", path: "/" }]
});
var insideRoot = (root, candidate) => {
...
var insidePortableRoot = (root, candidate) => candidate === root || candidate.startsWith(`${root}/`);
var sortedManifestErrors = (errors) => errors.sort((left, right) => compareCodePoints(left.path, right.path) || compareCodePoints(left.code, right.code));
...
var resolveListedPath = async (root, path) => {
  const resolved = resolve3(root, path);
  if (!insideRoot(root, resolved)) return null;
  try {
    const [realRoot, realCandidate] = await Promise.all([realpath4(root), realpath4(resolved)]);
...
var validateEntry = async (root, entry) => {
  ...
  const map = JSON.parse(await readFile3(mapPath, "utf8"));
  return validateBilingualPair(enPath, zhPath, map);
};
var validateFixturesUnchecked = async (rootValue) => {
  const root = resolve3(rootValue);
  let manifest;
  try {
    manifest = JSON.parse(await readFile3(resolve3(root, "manifest.json"), "utf8"));
  } catch {
    return {
      ok: false,
      results: [],
...
var validateFixtures = async (rootValue) => {
  try {
...
import { lstat as lstat5, mkdir, readFile as readFile5, realpath as realpath6, unlink as unlink2 } from "node:fs/promises";
import { isAbsolute as isAbsolute7, join as join5, relative as relative5, sep as sep5 } from "node:path";
...
var collectDeliveryInventory = async ({ lifecycleRoot: rootValue, overlays = {} } = {}) => {
  if (typeof rootValue !== "string" || !isAbsolute6(rootValue) || overlays === null || typeof overlays !== "object" || Array.isArray(overlays)) {
    return failure3("DELIVERY_INVENTORY_INVALID", "/", "A bounded lifecycle root and overlay map are required.");
  }
  let lifecycleRoot;
  try {
    const state = await lstat4(resolve4(rootValue));
    lifecycleRoot = await realpath5(resolve4(rootValue));
    if (!state.isDirectory() || state.isSymbolicLink()) throw new Error("Regular lifecycle root required.");
  } catch {
    return failure3("DELIVERY_INVENTORY_INVALID", "/lifecycleRoot", "A regular lifecycle root is required.");
  }
  const sources = /* @__PURE__ */ new Map();
  let entryCount = 0;
  const scan = async (rootLocator) => {
    const lexical = join4(lifecycleRoot, rootLocator);
    let root;
    try {
      const state = await lstat4(lexical);
      root = await realpath5(lexical);
      if (!state.isDirectory() || state.isSymbolicLink() || !inside2(lifecycleRoot, root)) throw new Error("Unsafe delivery root.");
    } catch (error) {
    ...
    const visit = async (directory, relativeDirectory, depth) => {
      ...
      for (const entry of await readdir2(directory, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          if (!inside2(lifecycleRoot, physical)) throw new Error("Delivery file escapes lifecycle root.");
          const state = await lstat4(physical);
  ...
  try {
    ...
    return failure3("DELIVERY_INVENTORY_INVALID", "/delivery", "Delivery inventory could not be collected safely.");
  }
  const markerSource = sources.get("delivery/layout.json");
  if (!markerSource) {
    return failure3("DELIVERY_LAYOUT_MIGRATION_REQUIRED", "/delivery/layout.json", "Delivery layout v2 marker is required.");
  }
  try {
    const source = markerSource.overlay ?? await readFile4(markerSource.path, "utf8");
    if (!validateJson("delivery-layout", JSON.parse(source)).ok) throw new Error("Invalid layout marker.");
  } catch {
    return failure3("DELIVERY_LAYOUT_MIGRATION_REQUIRED", "/delivery/layout.json", "Delivery layout v2 marker is invalid.");
  }
  ...
  if (presentViews[0] !== presentViews[1]) {
    return failure3("DELIVERY_INVENTORY_PAIR_INVALID", "/delivery/views", "Generated delivery views require both languages.");
  }
  if (presentViews[0]) {
    for (const locator of viewLocators) {
      const source = sources.get(locator);
      const text = source.overlay ?? (source.size <= 131072 ? await readFile4(source.path, "utf8").catch(() => null) : null);
      if (typeof text !== "string" || Buffer.byteLength(text) > 131072 || !text.startsWith(GENERATED_NOTICE)) {
        return failure3("DELIVERY_INVENTORY_INVALID", `/${locator}`, "Generated delivery view locator is occupied.");
      }
      ignored.add(locator);
    }
  }
  for (const [locator, source] of sources) {
    ...
    if (!canonicalIndex(locator)) {
      return failure3("DELIVERY_INVENTORY_INVALID", `/${locator}`, "Delivery index locator is not canonical.");
    }
    const content = source.overlay ?? (source.size <= 131072 ? await readFile4(source.path, "utf8").catch(() => null) : null);
    if (typeof content !== "string" || !content.startsWith(INDEX_NOTICE)) {
      return failure3("DELIVERY_INVENTORY_INVALID", `/${locator}`, "Delivery index locator is occupied.");
    }
    ignored.add(locator);
  }
  ...
  for (const [locator, source] of [...sources.entries()].sort(([left], [right]) => compareCodePoints(left, right))) {
    ...
    if (!descriptor || descriptor.expectedKind === null) {
      return failure3("DELIVERY_INVENTORY_INVALID", `/${locator}`, "Delivery contains an unknown managed file.");
    }
    ...
    if (!metadata || metadata.artifact_id !== descriptor.artifactId || metadata.artifact_kind !== descriptor.expectedKind || descriptor.ownerId !== null && metadata.owner_artifact_id !== descriptor.ownerId) {
      return failure3("DELIVERY_INVENTORY_PATH_MISMATCH", `/${locator}`, "Delivery path and Frontmatter ownership must match.");
    }
    ...
    if (!Object.values(expected).includes(locator)) {
      return failure3("DELIVERY_INVENTORY_PATH_MISMATCH", `/${locator}`, "Delivery locator is not canonical for its owner.");
    }
    ...
    if (pair[language]) return failure3("DELIVERY_INVENTORY_DUPLICATE", `/${locator}`, "Delivery artifact language is duplicated.");
  ...
  for (const [key, pair] of grouped) {
    if (!pair.en || !pair["zh-CN"] || !isDeepStrictEqual2(pair.en.frontmatter, pair["zh-CN"].frontmatter)) {
      return failure3("DELIVERY_INVENTORY_PAIR_INVALID", `/${key}`, "Delivery artifacts require one matching bilingual pair.");
    }
    const item = itemFromPair(pair);
    if (artifactIds.has(item.artifact_id)) {
      return failure3("DELIVERY_INVENTORY_DUPLICATE", `/${item.artifact_id}`, "Delivery artifact IDs must be globally unique.");
    }
...
var failure4 = (path, message) => fail([
...
var deriveAlignmentReview = ({ feedbacks = [], owners = [], closures = [] } = {}) => {
  if (!Array.isArray(feedbacks) || !Array.isArray(owners) || !Array.isArray(closures)) {
    return failure4("/", "Alignment review inputs must be bounded arrays.");
  }
  ...
  for (const [index, closure] of closures.entries()) {
    if (!validateClosureSummary(closure).ok || closureByOwner.has(closure.owner_artifact_id)) {
      return failure4(`/closures/${index}`, "Closure summaries require unique owner references.");
    }
  ...
  for (const [index, candidate] of owners.entries()) {
    ...
    if (!validation2.ok || !["prd", "non-prd-delivery"].includes(candidate.artifact_kind)) {
      return failure4(`/owners/${index}`, "Alignment owners must be valid PRD or non-PRD delivery assets.");
    }
  ...
  for (const [index, entry] of feedbacks.entries()) {
    ...
    if (!validation2.ok || entry.frontmatter.artifact_kind !== "feedback" || entry.frontmatter.retention_tier !== "active") {
      return failure4(`/feedbacks/${index}`, "Alignment entries must reference valid Feedback Frontmatter.");
    }
    const feedbackId = entry.frontmatter.artifact_id;
    if (ids.has(feedbackId)) return failure4(`/feedbacks/${index}`, "Feedback IDs must be unique.");
    ids.add(feedbackId);
    ...
    if (!validateJson("alignment-marker", entry.marker).ok || !entry.frontmatter.domain_ids.includes(entry.marker.primary_domain_id) || !safeTitle(entry.titles?.en) || !safeTitle(entry.titles?.["zh-CN"])) {
      return failure4(`/feedbacks/${index}`, "Active alignment Feedback must have one validated marker and localized titles.");
    }
    ...
    for (const candidate of linkedOwners) {
      ...
      if (ownerProject !== feedbackProject) {
        return failure4(`/owners/${candidate.artifact_id}`, "Linked alignment owner must belong to the Feedback project.");
      }
      const closure = closureByOwner.get(candidate.artifact_id);
      if (closure && closure.baseline.starting !== candidate.knowledge_baseline) {
        return failure4(`/closures/${candidate.artifact_id}`, "Owner closure must match its starting knowledge baseline.");
      }
      if (rejectedClosure(closure)) {
        if (!coversFeedback(closure, feedbackId)) {
          return failure4(`/closures/${candidate.artifact_id}`, "Terminal closure must explicitly cover linked Feedback.");
        }
        continue;
      }
      ...
      if (closure?.outcome?.status === "ACCEPTED") {
        if (!acceptedClosure(closure, feedbackId)) {
          return failure4(`/closures/${candidate.artifact_id}`, "Accepted closure must explicitly cover linked Feedback.");
        }
  ...
  return validation.ok ? ok(review) : failure4("/", "Derived alignment review violates its closed schema.");
};
...
var inside3 = (root, candidate) => {
  const path = relative5(root, candidate);
  return path === "" || path !== ".." && !path.startsWith(`..${sep5}`) && !isAbsolute7(path);
};
var regularDirectory = async (path, parent = null) => {
  const state = await lstat5(path);
  const physical = await realpath6(path);
  if (!state.isDirectory() || state.isSymbolicLink() || parent && !inside3(parent, physical)) {
    throw new Error("Unsafe alignment review directory.");
  }
  return physical;
};
var resolveLifecycleRoot = async (root) => {
  if (typeof root !== "string" || !isAbsolute7(root)) throw new Error("Absolute project root required.");
  const projectRoot = await regularDirectory(root);
  const docsRoot = await regularDirectory(join5(root, "docs"), projectRoot);
  const lifecycleRoot = await regularDirectory(join5(root, "docs", "project-lifecycle"), docsRoot);
  await regularDirectory(join5(root, "docs", "project-lifecycle", "delivery"), lifecycleRoot);
...
var splitDeliveryDocument2 = (source) => {
  const normalized = source.replaceAll("\r\n", "\n");
  if (!normalized.startsWith("---\n")) return null;
  const closing = normalized.indexOf("\n---\n", 4);
  if (closing === -1) return null;
...
var discoverAlignmentInventory = async (lifecycleRoot) => {
  ...
  for (const entry of [...collected.value.pairs, ...collected.value.archived_pairs]) {
    const path = join5(lifecycleRoot, entry.locator);
    const state = await lstat5(path);
    const physical = await realpath6(path);
    if (!state.isFile() || state.isSymbolicLink() || state.size > MAX_DOCUMENT_BYTES || !inside3(lifecycleRoot, physical)) {
      throw new Error("Delivery inventory contains an unsafe document.");
    }
    const document = splitDeliveryDocument2(await readFile5(physical, "utf8"));
    if (!document || !isDeepStrictEqual3(document.frontmatter, entry.frontmatter)) {
      throw new Error("Delivery inventory changed after validation.");
    }
    const pair = pairs.get(entry.frontmatter.artifact_id) ?? {};
    pair[entry.language] = document;
    pairs.set(entry.frontmatter.artifact_id, pair);
  }
  const feedbacks = [];
  const owners = [];
  ...
  const linkedOwners = owners.filter(({ relationships }) => relationships.feedback_ids.some((id) => activeFeedbackIds.has(id)));
...
var ensureProjectionDirectory = async (lifecycleRoot) => {
  const path = join5(lifecycleRoot, "delivery", "views");
  try {
    return await regularDirectory(path, lifecycleRoot);
  } catch (error) {
    if (error.code !== "ENOENT") throw error;
    await mkdir(path);
    return regularDirectory(path, lifecycleRoot);
  }
};
...
var validateAuthoritativeInventory = async (lifecycleRoot, input) => {
  ...
  const suppliedOwners = input.owners.filter(({ relationships }) => relationships.feedback_ids.some((id) => activeFeedbackIds.has(id)));
...
var writeContent = (write, lifecycleRoot, locator, content) => write({
  ...
  target: locator,
  content,
  validate: async (source) => source === content ? ok(source) : failure4("/", "Generated projection content changed during publication.")
});
var restore = async ({ write, remove, lifecycleRoot, locator, original }) => {
  if (original === null) {
    try {
      await remove(join5(lifecycleRoot, locator));
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
    }
    return;
...
var syncAlignmentReview = async (input = {}, operations = {}) => {
  ...
  const paths = {
    en: join5(lifecycleRoot, locators.en),
    "zh-CN": join5(lifecycleRoot, locators["zh-CN"])
  };
  let existing;
  try {
    existing = {
...
`))) {
  ...
  if (review.value.rows.length === 0) {
    ...
    try {
      ...
      return fail([createError("ALIGNMENT_REVIEW_WRITE_FAILED", "/delivery", "Projection pair removal failed; the generated pair is unchanged.")]);
    }
    try {
...
var redactFailureDiagnostics = (result) => {
...
var samePhysicalFile = async (left, right) => {
  try {
    const [leftReal, rightReal] = await Promise.all([realpath7(left), realpath7(right)]);
...
var resolvePointerMap = async (pointerFile, locator) => {
  ...
  const mapFile = resolve5(dirname4(resolve5(pointerFile)), locator);
  let source;
  try {
    source = await readFile6(mapFile, "utf8");
  } catch {
    return cliFailure(
      ERROR_CODES.REFERENCE_MISSING,
      "/governance_locator",
...
var hasStringGovernanceLocator = (value) => value !== null && typeof value === "object" && !Array.isArray(value) && typeof value.governance_locator === "string" && value.governance_locator.length > 0;
...
var isInside = (base, candidate) => {
  ...
  return fromBase === "" || !fromBase.startsWith(`..${sep6}`) && fromBase !== ".." && !isAbsolute8(fromBase);
};
...
if (command === "help") {
  ...
  } else if (!isAbsolute8(options["--root"]) || !isAbsolute8(options["--output"])) {
    emit(cliFailure("CLI_PATH_INVALID", "/arguments", "Root and output paths must be absolute."), 2);
  } else {
    try {
      const lexicalLifecycleRoot = resolve5(options["--root"], "docs/project-lifecycle");
      const lexicalOutput = resolve5(options["--output"]);
      if (isInside(lexicalLifecycleRoot, lexicalOutput)) {
        ...
        const root = await realpath7(options["--root"]);
        const outputParent = await realpath7(dirname4(options["--output"]));
        const outputName = basename3(options["--output"]);
        const output = resolve5(outputParent, outputName);
        const lifecycleRoot = resolve5(root, "docs/project-lifecycle");
        const physicalLifecycleRoots = [lifecycleRoot];
        try {
          physicalLifecycleRoots.push(await realpath7(lifecycleRoot));
        } catch (error) {
          if (error.code !== "ENOENT") throw error;
        }
...
`;
  ...
  if (!options || !isAbsolute8(options["--root"]) || !isAbsolute8(options["--input"])) {
      ...
      if (!input.ok) {
        ...
        if (kind === "project-pointer" && hasStringGovernanceLocator(value)) {
          ...
          if (unresolvedResult.errors.some(({ code }) => code === ERROR_CODES.SCHEMA_INVALID)) {
            ...
            result = resolved.ok ? validateJson(kind, value, { resolvedProjectMap: resolved.value }) : resolved;
          }
        } else {
