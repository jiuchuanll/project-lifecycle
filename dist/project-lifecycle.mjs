...
var discoverAlignmentInventory = async (lifecycleRoot) => {
  ...
  for (const [id, pair] of pairs) {
    ...
    if (frontmatter.artifact_kind === "feedback" && frontmatter.retention_tier === "active") {
      ...
      closureIds.add(frontmatter.artifact_id);
    }
  }
  ...
  return {
    feedbacks,
    owners: linkedOwners,
    closureIds: new Set([...closureIds].filter((id) => id.startsWith("closure-") && linkedOwnerIds.has(id.slice("closure-".length))))
  };
};
...
var validateAuthoritativeInventory = async (lifecycleRoot, input) => {
  ...
  return sameRecords(suppliedFeedbacks, inventory.feedbacks, ({ frontmatter }) => frontmatter.artifact_id) && sameRecords(suppliedOwners, inventory.owners, ({ artifact_id: id }) => id) && suppliedClosures.length === inventory.closureIds.size && suppliedClosures.every(({ artifact_id: id }) => inventory.closureIds.has(id));
};
...
`;
          ...
          emit(ok({
            ...
            content_hash: `sha256:${createHash2("sha256").update(content).digest("hex")}`
          }));
        }
      }
    } catch (error) {
