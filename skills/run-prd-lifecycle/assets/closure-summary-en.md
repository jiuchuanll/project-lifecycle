---
schema_version: 1
artifact_id: closure-summary-template
artifact_kind: closure-summary
primary_route: PRD_DELIVERY
project_id_at_creation: sample-project
current_project_id: sample-project
domain_ids: [sample-domain]
knowledge_baseline: replace-with-baseline
relationships: { feedback_ids: [], prd_ids: [], legacy_artifact_refs: [] }
retention_tier: active
reclassified_from_refs: []
obligations: []
---
# Closure summary

## Outcome and acceptance
Record the immutable compact result and accepting authority.

## Coverage and evidence
Link exact criteria, Feedback coverage, verification, and residual risk.

## Knowledge handoff and retention
Reference the Knowledge Diff or no-change result and final retention state.
