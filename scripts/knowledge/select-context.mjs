...
const readBoundedNavigationIndex = async (path, maxBytes = 262_144) => {
  const state = await lstat(path);
  if (state.size > maxBytes) {
    throw Object.assign(new Error('Navigation index exceeds the bounded read limit.'), { code: 'CONTEXT_INDEX_INVALID' });
  }
  const source = await readFile(path, 'utf8');
  if (!source.startsWith('<!-- Generated by Project Lifecycle from project-map and validated Frontmatter; do not edit. -->\n')) {
    throw Object.assign(new Error('Generated navigation index required.'), { code: 'CONTEXT_INDEX_INVALID' });
  }
...
export async function selectContext(inputValue, operations = {}) {
  ...
  if (map.schema_version !== 2) {
    return failure(
      'CONTEXT_LAYOUT_MIGRATION_REQUIRED',
      '/schema_version',
      'Project knowledge must be migrated to the v2 recursive layout before knowledge reads.',
    );
  }
  ...
  for (const domain of map.domains) {
    if (domain.domain_state === 'materialized'
      && !samePair(domain.paired_assets, pairForDomain(planned.value, domain.id))) {
      return failure(
        'CONTEXT_LAYOUT_INVALID',
        `/domains/${domain.id}/paired_assets`,
        'Knowledge routing requires the canonical recursive v2 layout; migrate the legacy layout before reading knowledge.',
      );
    }
  }
