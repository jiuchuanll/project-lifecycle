...
const discoverAlignmentInventory = async (lifecycleRoot) => {
  ...
  return {
    feedbacks,
    owners: linkedOwners,
    closureIds: new Set([...closureIds].filter((id) => (
      id.startsWith('closure-') && linkedOwnerIds.has(id.slice('closure-'.length))
    ))),
  };
};
...
const validateAuthoritativeInventory = async (lifecycleRoot, input) => {
  ...
  return sameRecords(suppliedFeedbacks, inventory.feedbacks, ({ frontmatter }) => frontmatter.artifact_id)
    ...
    && suppliedClosures.length === inventory.closureIds.size
    && suppliedClosures.every(({ artifact_id: id }) => inventory.closureIds.has(id));
};
