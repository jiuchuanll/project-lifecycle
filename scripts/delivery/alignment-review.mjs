...
import { alignmentReviewPair } from './delivery-layout.mjs';
...
export { closureSummaryHash } from '../lib/closure-summary.mjs';
...
export const deriveAlignmentReview = ({ feedbacks = [], owners = [], closures = [] } = {}) => {
  ...
  for (const [index, entry] of feedbacks.entries()) {
    const validation = validateJson('delivery-frontmatter', entry?.frontmatter);
    if (!validation.ok || entry.frontmatter.artifact_kind !== 'feedback'
      || entry.frontmatter.retention_tier !== 'active') {
    ...
    if (!validateJson('alignment-marker', entry.marker).ok
      || !entry.frontmatter.domain_ids.includes(entry.marker.primary_domain_id)
      || !safeTitle(entry.titles?.en) || !safeTitle(entry.titles?.['zh-CN'])) {
    ...
    const alignmentPhase = ownerRef.length > 0 && !allAccepted
      ...
      : allAccepted
        ? 'KNOWLEDGE_WRITEBACK'
        : entry.marker.routing_disposition === 'DEFERRED'
          ? 'DEFERRED'
          : 'REVIEW_REQUIRED';
    rows.push({
...
const discoverAlignmentInventory = async (lifecycleRoot) => {
  ...
  for (const entry of [...collected.value.pairs, ...collected.value.archived_pairs]) {
    const path = join(lifecycleRoot, entry.locator);
    const state = await lstat(path);
    const physical = await realpath(path);
    if (!state.isFile() || state.isSymbolicLink() || state.size > MAX_DOCUMENT_BYTES
      || !inside(lifecycleRoot, physical)) {
      throw new Error('Delivery inventory contains an unsafe document.');
    }
    const document = splitDeliveryDocument(await readFile(physical, 'utf8'));
    if (!document || !isDeepStrictEqual(document.frontmatter, entry.frontmatter)) {
  ...
  for (const [id, pair] of pairs) {
    ...
    if (frontmatter.artifact_kind === 'feedback') {
      ...
      const containsAlignmentSyntax = Object.values(bodies)
        .some((body) => body.includes('<!-- project-lifecycle:alignment\n'));
...
const ensureProjectionDirectory = async (lifecycleRoot) => {
  const path = join(lifecycleRoot, 'delivery', 'views');
  try {
