...
test('builds a deterministic private candidate with the complete explicit release surface', async (context) => {
  ...
  assert.deepEqual(
    firstBytes,
    await readFile(join(repositoryRoot, 'dist/project-lifecycle-0.6.0.zip')),
    'checked-in archive must equal a clean deterministic rebuild',
  );
  assert.equal(
    await readFile(join(repositoryRoot, 'dist/project-lifecycle-0.6.0.zip.sha256'), 'utf8'),
    `${first.value.sha256}  project-lifecycle-0.6.0.zip\n`,
  );
  ...
  const prefix = 'project-lifecycle-0.6.0/';
...
test('refuses to follow an existing release output symlink', async (context) => {
  ...
  const archivePath = join(outputDirectory, 'project-lifecycle-0.6.0.zip');
