...
const validTrace = () => ({
  ...
  run_number: 1,
  plugin: { version: '0.7.0', commit: 'a'.repeat(40) },
...
test('passes static package conformance without host-local Skill copies', async () => {
  ...
  assert.deepEqual(result.value.host_ids, ['claude', 'codex', 'cursor', 'dsh', 'kimi', 'zcode']);
  assert.equal(result.value.bundle_version, '0.7.0');
});
